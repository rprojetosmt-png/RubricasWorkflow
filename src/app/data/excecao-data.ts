...
export const orgaosExcecao: OpcaoFiltro[] = [
...
export const categoriasExcecao: OpcaoFiltro[] = [
...
export const cargosExcecao: OpcaoFiltro[] = [
...
export function getFilterLabels(filtros: Excecao["filtros"]) {
  ...
  if (filtros.orgaos.length)
    result.push({
      campo: "Órgão",
      valores: filtros.orgaos.map((id) => orgaosExcecao.find((o) => o.id === id)?.label ?? id).join(", "),
    });
  if (filtros.tiposVinculo.length)
    result.push({
      campo: "Tipo de Vínculo",
      valores: filtros.tiposVinculo.map((id) => tiposVinculo.find((o) => o.id === id)?.label ?? id).join(", "),
    });
  if (filtros.categorias.length)
    result.push({
      campo: "Categoria",
      valores: filtros.categorias.map((id) => categoriasExcecao.find((o) => o.id === id)?.label ?? id).join(", "),
    });
  if (filtros.cargos.length)
    result.push({
      campo: "Cargo",
      valores: filtros.cargos.map((id) => cargosExcecao.find((o) => o.id === id)?.label ?? id).join(", "),
    });
  if (filtros.situacoesFuncionais.length)
    result.push({
      campo: "Situação Funcional",
      valores: filtros.situacoesFuncionais.map((id) => situacoesFuncionais.find((o) => o.id === id)?.label ?? id).join(", "),
    });
  if (filtros.servidores.length)
    result.push({
      campo: "Pessoa",
      valores: filtros.servidores
        .map((id) => {
          const s = servidores.find((o) => o.id === id);
          return s ? `${s.matricula} - ${s.nome}` : id;
        })
        .join(", "),
    });
